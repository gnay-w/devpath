...
|------|------|------|
| 语法手感 | 能读能猜 | P0 快速对齐即可 |
| 指针 / 逃逸 | 偏 C/Java 对象直觉 | P1 专练 escape analysis |
| slice | 底层数组共享不熟 | P1 首攻 |
| interface / nil | 明显缺口 | P1 专练 |
| 并发 | 有直觉，细节/版本差待补 | P2 |
| HTTP / context | 工程直觉在线 | P3 直接上项目强度 |
| Java 迁移 | ThreadLocal / 线程池等可迁移 | 每阶段显式对照 |
...
**下一步：** 写 `notes/java-to-go.md`（P0），然后开 `labs/001-slice-and-interface/`。
